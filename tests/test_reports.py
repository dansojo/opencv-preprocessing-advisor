...
def test_image_report_contains_reproducibility_metadata(tmp_path, sample_bgr):
    ...
    changes = pd.read_csv(output / "metric_changes.csv")
    ...
    assert {"before", "after", "absolute_delta", "percent_delta"} <= set(changes.columns)
...
def test_benchmark_report_contains_detailed_metrics_and_confusion_matrices(tmp_path):
    ...
    assert metadata["actual_folds"] == 3
    assert len(metadata["samples"]) == 12
    assert len(metadata["fold_assignments"]) == 3
